...
public class Game {
    ...
    private List<Integer> input() {
        ...
        if (!Validation.isContainZero(Integer.parseInt(input))) {
            throw new IllegalArgumentException("0은 게임에서 사용되지 않는 숫자입니다.");
        }
    ...
    private void out() {
        ...
        if (count.get(0) == 3) {
            ...
            if (!Validation.isAnswer(restartQuestion)) {
                throw new IllegalArgumentException("입력이 잘못되었습니다.");
            }

            this.reStart = restartQuestion.equals("1");
        }
    }
