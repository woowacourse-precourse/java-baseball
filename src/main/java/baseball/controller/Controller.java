...
public class Controller {
    ...
    private boolean playGameOnce(Computer computer, User user){
        String userInput = View.askUserNumber();
        validateUserInput(userInput, BASEBALL_GAME_NUMBER_LENGTH);
        user.saveUserInput(userInput);
    ...
    private boolean askRestartOrExit() {
        View.printCorrectAnswerMessage(BASEBALL_GAME_NUMBER_LENGTH);

        String response_RestartOrExit = View.askRestartOrExit(USER_CHOICE_SENTENCE);
        validateAskRestartOrExitInput(response_RestartOrExit, GAME_RESTART, GAME_EXIT, GAME_RESTART_OR_EXIT_RESPONSE_LENGTH);
        if(Integer.parseInt(response_RestartOrExit) == GAME_EXIT)
            return false;
        else if(Integer.parseInt(response_RestartOrExit) == GAME_RESTART)
            return true;
        else
