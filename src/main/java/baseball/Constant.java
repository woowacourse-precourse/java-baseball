package baseball;

public class Constant {
    public static String GAME_START = "숫자 야구 게임을 시작합니다";
    public static String GAME_FINISH = "게임 종료";
    public static String CORRECT = "3개의 숫자를 모두 맞히셨습니다! ";
    public static String WRITE_ANSWER = "숫자를 입력해주세요 : ";
    public static String STRIKE = "스트라이크";
    public static String BALL = "볼";
    public static String NOTHING = "낫싱";
    public static String PERFECT = "3스트라이크";
    public static String OPTION = "게임을 새로 시작하려면 1, 종료하려면 2를 입력하세요.";
    public static String NEW_GAME = "1";
    public static String QUIT = "2";
}
