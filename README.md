...
- [x] 플레이어는 3자리수를 입력한다.
  ...
  - [x] 플레이어가 입력한 숫자를 가공하여 Balls 객체로 만든다.
    - [x] String으로 입력받은 문자를 숫자 List로 변환한다.
    - [x] List 안에 있는 숫자들을 Ball 객체로 만든다.
    - [x] 만들어진 Ball 객체를 Balls 라는 일급 컬렉션안에 넣는다.
...
- [x] 플레이어가 입력한 3자리수가 컴퓨터가 랜덤으로 만든 3자리수와 일치하면 게임에서 승리한다.
  - [x] 게임에서 승리하면 게임을 계속 시작할지 게임을 종료할지 결정할 수 있게 한다.  
    - [x] 플레이어가 1을 입력하면 게임이 재시작된다.
    - [x] 플레이어가 2를 입력하면 게임이 종료된다.
    - [x] 1(Continue) 또는 2(Stop) 말고 다른 문자를 입력했으면 에러가 발생한다.
