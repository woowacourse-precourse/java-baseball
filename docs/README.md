...
### I-1. 초기화
- 정답 설정
  * 난수 발생: 일, 십, 백의 자리수 중 중복되는 수가 있다면 난수 재발생
...
- `n스트라이크`: '같은 수가 같은 자리에 있다'
  ...
  * `3스트라이크`의 경우 입력과 정답이 일치하는 경우로, [게임 종료 단계](#iii-게임-종료-단계)로 이동
- `n볼`: '수가 다른 자리에 있다'
...
### IV-2. 예외처리
- 사용자가 [잘못된 값](#iv-1-잘못된-값의-정의)을 입력할 경우 `IllegalArgumentException`을 발생시킨 후 애플리케이션 종료