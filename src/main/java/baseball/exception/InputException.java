...
public class InputException {
    ...
    public static String isValidInput(String userInput) {
        if (!isLengthThree(userInput)) throw new IllegalArgumentException();
        if (!isThreeDifferentDigits(userInput) || !isNumberBetweenOneAndNine(userInput)) throw new IllegalArgumentException();
        return userInput;
    }
    ...
    public static boolean isThreeDifferentDigits(String userInput) {
        ...
        if (firstDigit == secondDigit || firstDigit == thirdDigit || secondDigit == thirdDigit) return false;
        return true;
    }
