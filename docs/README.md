...
- 테스트 함수
  1. `judgeEachNumber_볼_확인()` : `볼`이 반환되어야 하는 모든 자릿수 조합에 대해 `볼`이 나오는지 확인한다.
  2. `judgeEachNumber_스트라이크_확인()` : `스트라이크`가 반환되어야 하는 모든 자릿수 조합에 대해 `스트라이크`가 나오는지 확인한다.
  3. `judgeEachNumber_낫싱_확인()` : `낫싱`이 반환되어야 하는 모든 자릿수 조합에 대해 `낫싱`이 나오는지 확인한다.
...
- 테스트 함수
  1. `overallTest_많은_출력()` : 여러 판의 게임을 진행하며 원하는 결과를 보유한지 확인한다.
  2. `overallTest_적은_출력()` : 존재할 수 없는 출력이 존재하지 않음을 확인한다.
